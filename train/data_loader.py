...
def Data_loader(ROOT_PATH,BATCH_SIZE=8):
    ...
    valid_loader = torch.utils.data.DataLoader(validset, batch_size = BATCH_SIZE // 4, shuffle = True, num_workers = 0)
    
    testset = Dataset(root_path = ROOT_PATH, mode = 'test', transforms = test_transforms)
    test_loader = torch.utils.data.DataLoader(testset, batch_size = BATCH_SIZE // 4, shuffle = False, num_workers = 0)
