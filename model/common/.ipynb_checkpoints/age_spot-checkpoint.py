...
def age_spot_analysis(folder_location, general_image, eval_image, face, device):
    
    s3_client = MyS3Client()

    model = unet(outchannels=1).to(device)
    model.load_state_dict(
        torch.load('weights/age_spot_Unet_84_0.368.pth', map_location=torch.device('cpu'))['net'])
    ...
    age_spot_score = round(len(mask_[mask_ != 0]) / len(resized_face[resized_face != 0]) * 100, 2)

    return Result(
        score=age_spot_score,
