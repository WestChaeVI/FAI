...
def texture_analysis(folder_location, general_image, eval_image, face, device):
    
    s3_client = MyS3Client()

    model = unet(outchannels=1).to(device)
    model.load_state_dict(
        torch.load('weights/texture_Unet_185_0.357.pth', map_location=torch.device('cpu'))['net'])
    ...
    texture_score = round(len(mask_[mask_ != 0]) / len(resized_face[resized_face != 0]) * 100, 2)
