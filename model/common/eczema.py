...
def eczema_analysis(input_data_path, folder_location,  general_image, face, alpha = 0.5):
    
    s3_client = MyS3Client()
    
    H,W,C = general_image.shape
    
    model = YOLO('weights/yolov8_seg_eczema_best.pt')
    ...
    if mask is None:
        ...
        eczema_score = round(len(mask_1[mask_1 != 0]) / len(resized_face[resized_face != 0]) * 100, 2)
        ...
        eczema_image = cv2.resize(general_image, (W, H), interpolation=cv2.INTER_LINEAR)

    data = PILImage.fromarray(eczema_image)
    s3_client.upload(folder=folder_location, file=data)
    
    return Result(score = eczema_score,
